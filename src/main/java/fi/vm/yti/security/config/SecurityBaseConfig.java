...
public class SecurityBaseConfig extends KeycloakWebSecurityConfigurerAdapter {
    ...
    UserDetailsService keycloakUserDetailsService() {
        return new KeycloakUserDetailsService(this.groupmanagementUrl);
    }

    // @Bean
    // Filter authenticationFilter() throws Exception {

    // final RequestAttributeAuthenticationFilter authenticationFilter = new
    ...
    // // need to update principal in any case since organizations might have
    // changed
    // return true;
    // }
    // };
    ...
    // authenticationFilter.setAuthenticationSuccessHandler((request, response,
    // authentication) -> {
    ...
    // final OncePerRequestFilter tokenAuthenticationFilter = new
    ...
    // response.setHeader("Set-Cookie", HEADER_YTITOKEN +
    // "=deleted;path=/;HttpOnly;expires=Thu, 01 Jan 1970 00:00:00 GMT");
    ...
    // headerString.startsWith("Bearer ")) {
    ...
    // private YtiUser getUserForToken(final String token) throws
    ...
    // final ResponseEntity<User> validateResponse =
    // restTemplate.postForEntity(validateTokenUri, tokenRequest, User.class);
    ...
    // user.tokenInvalidationAt, rolesInOrganizations, user.containerUri,
    // user.tokenRole);
    // }
    // }
    ...
    // final PreAuthenticatedAuthenticationProvider authenticationProvider = new
    ...
    protected void configure(final AuthenticationManagerBuilder auth) throws Exception {
        ...
        KeycloakUserDetailsAuthenticationProvider keycloakAuthenticationProvider = new KeycloakUserDetailsAuthenticationProvider();
        keycloakAuthenticationProvider.setUserDetailsService(keycloakUserDetailsService());
    ...
    protected void configure(final HttpSecurity http) throws Exception {
        ...
        // // Don't inject fake user for public-api or otherwise it will end up in
        // infinite loop
        ...
        // private @Nullable FakeUserLogin resolveFakeUserLogin(final HttpServletRequest
        // request) {
        ...
        // } else if (!user.isAnonymous()) { // keep previously logged in user still
        // logged in
        // return new FakeUserLogin(user.getEmail(), user.getFirstName(),
        // user.getLastName());
        ...
        http.antMatcher("/**/*")
                ...
                // RequestAttributeAuthenticationFilter.class)
                // .addFilterBefore(tokenAuthenticationFilter(),
                // RequestAttributeAuthenticationFilter.class)
                // .addFilterBefore(fakeUserSettingFilter,
                // RequestAttributeAuthenticationFilter.class)
